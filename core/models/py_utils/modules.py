...
class hg_net(nn.Module):
    ...
    def _test(self, *xs, **kwargs):
        ...
        if config_debug.visualize_jh:
            tl_mod, p1_conv1, p2_conv1, pool1, pool2, p_bn1, concat = self.tl_modules[-1](cnvs[-1])
            br_mod, p1_conv12, p2_conv12, pool12, pool22, p_bn12, concat2 = self.br_modules[-1](cnvs[-1])
        else:
        ...
        if config_debug.visualize_jh:
            return self._decode(*outs, **kwargs), tl_heat, br_heat, tl_tag, br_tag, tl_mod, p1_conv1, p2_conv1, pool1, pool2, p_bn1, concat, tl_heat, cnvs[-1]
        else:
    ...
    def _test(self, *xs, **kwargs):
        ...
        if config_debug.visualize_jh:
            return self._decode(*outs,
                                **kwargs), t_heat, l_heat, b_heat, r_heat, t_tag, l_tag, b_tag, r_tag, t_mod, l_mod, b_mod, r_mod, \
...
class line_net_tlbr(nn.Module):
    def __init__(
            self, hg, tlbr_modules, t_heats, l_heats, b_heats, r_heats,
            t_tags, l_tags, b_tags, r_tags, t_offs, l_offs, b_offs, r_offs
    ):
        super(line_net_tlbr, self).__init__()
