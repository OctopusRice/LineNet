...
def cornernet_ifp_saccade(db, nnet, result_dir, debug=False, decode_func=batch_decode):
    ...
    for k_ind in tqdm(range(0, num_images), ncols=80, desc="locating kps"):
        ...
        timer.tic()
        top_bboxes[image_id] = cornernet_ifp_saccade_inference(db, nnet, image)
        timer.toc()
...
def cornernet_ifp_saccade_inference(db, nnet, image, decode_func=batch_decode):
