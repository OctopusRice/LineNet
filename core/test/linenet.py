...
def linenet(db, nnet, result_dir, debug=False, decode_func=decode):
    ...
    for ind in tqdm(range(0, num_images), ncols=80, desc="locating kps"):
        ...
        timer.tic()
        top_bboxes[image_id] = linenet_inference(db, nnet, image)
        timer.toc()
...
def linenet_inference(db, nnet, image, decode_func=decode):
    K             = db.configs["top_k"]
